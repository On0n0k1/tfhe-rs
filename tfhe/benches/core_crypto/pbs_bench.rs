...
mod cuda {
    ...
    fn cuda_pbs<Scalar: UnsignedTorus + CastInto<usize> + Serialize>(c: &mut Criterion) {
        ...
        for (name, params) in benchmark_parameters::<Scalar>().iter() {
            ...
            unsafe {
                d_input_indexes.copy_to_gpu_async(h_indexes.as_ref(), &stream);
                d_output_indexes.copy_to_gpu_async(h_indexes.as_ref(), &stream);
                d_lut_indexes.copy_to_gpu_async(h_indexes.as_ref(), &stream);
            }
            stream.synchronize();
