...
int c_threads_callback(void *vdata) {
  CThreadData *request = vdata;

  return tfhe_threading_context_run(request->context, do_work_in_tfhe_threading_context,
                                    (void *)request->data);
}
...
void do_thing(const ClientKey *client_key, const ServerKey *server_key) {
  ...
  thrd_t c_threads[NUM_C_THREADS] = {0};
  ...
  for (size_t i = 0; i < NUM_C_THREADS; ++i) {
    ...
    status = thrd_create(&c_threads[i], c_threads_callback, &c_callback_data[i]);
    assert(status == thrd_success);
  }

  for (size_t i = 0; i < NUM_C_THREADS; ++i) {
    int thrd_status = thrd_success;
    status = thrd_join(c_threads[i], &thrd_status);
    assert(thrd_status == thrd_success && status == thrd_success);
  }

  for (size_t i = 0; i < NUM_C_THREADS; ++i) {
    uint64_t clear_result;
    status = fhe_uint64_decrypt(data[i].result, client_key, &clear_result);
    assert(status == 0);
