...
impl From<CiphertextModulusCreationError> for &str {
    fn from(value: CiphertextModulusCreationError) -> Self {
        match value {
            CiphertextModulusCreationError::ModulusTooBig => {
                "Modulus is bigger than the maximum value of the associated Scalar type"
            }
            CiphertextModulusCreationError::CustomModuli64BitsOrLessOnly => {
...
impl CiphertextModulusCreationError {
    pub const fn const_panic(self) -> ! {
        match self {
            Self::ModulusTooBig => {
                panic!("Modulus is bigger than the maximum value of the associated Scalar type")
            }
            Self::CustomModuli64BitsOrLessOnly => {
                panic!("Non power of 2 moduli are not supported for types wider than u64")
            }
        }
    }
}
