...
impl<T: Numeric> CudaVec<T> {
    ...
    pub unsafe fn from_async(src: &Vec<T>, stream: &CudaStream) -> Self {
        ...
        if !src.is_empty() {
            res.copy_to_gpu_async(src, stream);
        }
        res
    }
    ...
    pub unsafe fn copy_to_gpu_async(&mut self, src: &[T], stream: &CudaStream)
    where
        T: Numeric,
    {
        let src_size = std::mem::size_of_val(src);
        assert!(self.len() * std::mem::size_of::<T>() >= src_size);
    ...
    /// - [CudaStream::synchronize] __must__ be called after the copy
    /// as soon as synchronization is required
    pub unsafe fn copy_gpu_to_gpu_async(&mut self, src: &Self, stream: &CudaStream)
    where
        T: Numeric,
    {
    ...
    where
        T: Numeric,
    {
        let src_size = self.len() * std::mem::size_of::<T>();
    ...
    pub(crate) fn range_bounds_to_start_end<R>(&self, range: R) -> (usize, usize)
    where
        R: std::ops::RangeBounds<usize>,
    {
